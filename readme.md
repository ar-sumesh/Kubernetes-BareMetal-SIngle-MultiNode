...
```
sudo kubeadm init --pod-network-cidr=192.168.0.0/16 --apiserver-advertise-address=<master server ip address> --node-name master
```
...
### If you want single node cluster then run the below commands and skip the worker node steps
...
```
sudo kubeadm join <master node ip address>:6443 --token xxxxx --discovery-token-ca-cert-hash sha256:xxx
```
